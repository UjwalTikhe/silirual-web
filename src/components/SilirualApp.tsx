...
function Welcome({ onNext }: { onNext: (mode: "using" | "helping") => void }) {
  return <main className="welcome-screen">
    <div className="welcome-photo"><img src={northeastWelcome} alt="Green hills, river and a traditional home in North East India" width={1200} height={900} /><div className="welcome-overlay"><div className="welcome-top"><Brand /><button className="language-pill"><Languages size={20} /> English <ChevronRight size={18} /></button></div><div className="welcome-message"><span className="official-label"><ShieldCheck size={18} /> A safe everyday companion</span><h1>Good days begin with a little support.</h1><p>Activities, memories and gentle reminders—made simple for you and the people who care.</p></div></div></div>
...
function Onboarding({ onDone, textSize, setTextSize }: { onDone: (name: string) => void; textSize: TextSize; setTextSize: (s: TextSize) => void }) {
  const [step, setStep] = useState(1); const [name, setName] = useState(""); const [settings, setSettings] = useState({ sound: true, haptic: true, voice: false, contrast: false, motion: false, memories: true, reminders: true, location: false, recording: false });
  ...
  return <main className="simple-screen woven-top onboarding-screen"><div className="progress-label"><span>Step {step} of 4</span><strong>{step * 25}% complete</strong></div><div className="progress-track"><span style={{width: `${step * 25}%`}} /></div>
    {step === 1 && <><PageHeader title="Choose your language" subtitle="You can change this at any time." /><div className="language-grid">{["English", "हिन्दी", "অসমীয়া", "বাংলা", "नेपाली", "মৈতৈলোন্"].map((l, i) => <button className={cn("language-card", i === 0 && "selected")} key={l}>{i === 0 && <Check />}<strong>{l}</strong><small>{["English","Hindi","Assamese","Bengali","Nepali","Manipuri"][i]}</small></button>)}</div></>}
...
function PicturePairs({ onClose }: { onClose: () => void }) {
  ...
  if(phase === "intro") return <div className="game-screen"><PageHeader title="Picture Pairs" subtitle="A gentle matching activity" onBack={onClose} /><div className="game-hero-icon"><Flower2 /></div><div className="instruction-card"><BookOpen /><div><h2>How to play</h2><p>Tap two cards. If the pictures are the same, you have found a pair.</p></div></div><div className="example-pairs"><span><Flower2 /></span><span><Flower2 /></span><strong>These match!</strong></div><ActionButton variant="secondary"><Volume2 /> Hear instructions</ActionButton><ActionButton onClick={() => setPhase("play")}><Play /> Start activity</ActionButton></div>;
...
function GamesView({ onGame }: { onGame: () => void }) { return <><PageHeader title="Games" subtitle="Choose a gentle activity. There is no time limit." /><div className="daily-streak"><Star /><div><strong>A little each day</strong><p>You enjoyed 3 activities this week.</p></div></div><div className="game-list">{games.map((g,i)=><button className="game-card" key={g.name} onClick={i===0?onGame:undefined}><span className="game-emoji">{g.icon}</span><span><strong>{g.name}</strong><small>{g.desc}</small><em><CloudOff /> {g.level} · Works offline</em></span><ChevronRight /></button>)}</div></>; }

function TodayView({ name, navigate, onGame }: { name:string; navigate:(v:ElderView)=>void; onGame:()=>void }) { return <><PageHeader title={`Namaste, ${name}`} subtitle="Tuesday, 15 September 2026" action={<button className="avatar-button" onClick={()=>navigate("settings")} aria-label="Open settings">{name[0]?.toUpperCase()}</button>} /><section className="feature-activity"><div><span className="section-label"><Sun /> Today’s activity</span><h2>Find the matching pictures</h2><p>A calm 5-minute activity to enjoy at your own pace.</p><ActionButton variant="warm" onClick={onGame}><Play /> Start activity</ActionButton></div><span className="feature-illustration"><Flower2 /></span></section><section><div className="section-heading"><h2>Also for today</h2><button onClick={()=>navigate("games")}>See all</button></div><div className="mini-grid"><button className="mini-card" onClick={()=>navigate("games")}><span><Shapes /></span><strong>Shape Match</strong><small>About 4 minutes</small></button><button className="mini-card" onClick={()=>navigate("games")}><span><MapPin /></span><strong>Remember Place</strong><small>About 5 minutes</small></button></div></section><div className="info-row"><button onClick={()=>navigate("reminders")}><IconBadge tone="gold"><Bell /></IconBadge><span><strong>2 reminders today</strong><small>Next at 6:00 PM</small></span><ChevronRight /></button><button onClick={()=>navigate("memories")}><IconBadge tone="green"><ImageIcon /></IconBadge><span><strong>A familiar memory</strong><small>Springtime in Shillong</small></span><ChevronRight /></button></div><section><div className="section-heading"><h2>Your routine</h2><button onClick={()=>navigate("routine")}>View plan</button></div><div className="routine-preview"><span className="done"><Check /></span><div><strong>Morning walk</strong><small>Completed at 8:30 AM</small></div><span className="next">Next</span></div></section></>; }

function MemoriesView() { return <><PageHeader title="Memories" subtitle="Familiar moments shared with care." /><article className="memory-card"><div className="memory-photo" role="img" aria-label="Green hills and a traditional home in North East India"><img src={northeastWelcome} loading="lazy" width={1200} height={900} alt="Green hills and a traditional home in North East India"/><span><Check /> Shared with you</span></div><div><p>12 September 2026</p><h2>Springtime in the hills</h2><p>We visited this peaceful valley together after the rain. You loved the red flowers by the path.</p><button><Volume2 /> Listen to this story</button></div></article><div className="empty-soft"><ImageIcon /><h2>More memories are on their way</h2><p>Your family can share familiar photos and stories with you.</p></div></>; }

function RemindersView() { const [done,setDone]=useState<number[]>([]); return <><PageHeader title="Reminders" subtitle="Your simple plan for today." /><div className="success-banner"><Check /> Reminders are ready on this device.</div>{[{time:"9:00 AM",title:"Morning medicine",desc:"After breakfast"},{time:"6:00 PM",title:"Call Priya",desc:"A friendly evening call"}].map((r,i)=><article className={cn("reminder-card",done.includes(i)&&"completed")} key={r.title}><div className="reminder-time"><Clock3 />{r.time}</div><h2>{r.title}</h2><p>{r.desc}</p><div><ActionButton onClick={()=>setDone([...done,i])}><Check /> {done.includes(i)?"Done":"Mark done"}</ActionButton><ActionButton variant="secondary"><Clock3 /> Snooze 15 min</ActionButton></div></article>)}</>; }

function ActivitiesView() { return <><PageHeader title="Activities" subtitle="Plan your day, one simple step at a time." action={<button className="icon-button"><Plus /></button>} /><div className="tabs-line"><button className="active">Planned · 3</button><button>Finished · 1</button></div>{[{t:"8:30 AM",n:"Morning walk",i:<Footprints/>,s:"Completed"},{t:"11:00 AM",n:"Picture Pairs",i:<Flower2/>,s:"Ready"},{t:"4:30 PM",n:"Listen to music",i:<Music/>,s:"Later today"}].map(a=><article className="activity-card" key={a.n}><span>{a.i}</span><div><small>{a.t} · {a.s}</small><h2>{a.n}</h2></div><ActionButton variant="secondary">{a.s==="Completed"?"Plan again":"Start"}</ActionButton></article>)}</>; }

function RoutineView() { const routine=[["Start the day","Morning walk and breakfast",<Sun/>],["Play a game","Picture Pairs is ready",<Flower2/>],["Enjoy a memory","Springtime in the hills",<ImageIcon/>],["Evening reminder","Call Priya at 6:00 PM",<Bell/>]] as const; return <><PageHeader title="My daily routine" subtitle="A comfortable rhythm for your day." /><div className="timeline">{routine.map((x,i)=><div key={x[0]}><span className={i===0?"complete":""}>{i===0?<Check />:i+1}</span><article><b>{x[2]}</b><div><h2>{x[0]}</h2><p>{x[1]}</p></div><ChevronRight /></article></div>)}</div></>; }

function HelpView({navigate}:{navigate:(v:ElderView)=>void}) { return <><PageHeader title="Help" subtitle="Support is always close by." /><ActionButton className="call-button"><Phone /> Call a trusted person</ActionButton><div className="help-list"><button><IconBadge><Volume2 /></IconBadge><span><strong>How to use SILIRUAL</strong><small>Hear a simple step-by-step guide</small></span><ChevronRight /></button><button onClick={()=>navigate("care")}><IconBadge tone="green"><HeartHandshake /></IconBadge><span><strong>My care circle</strong><small>People connected with you</small></span><ChevronRight /></button><button onClick={()=>navigate("location")}><IconBadge tone="gold"><LocateFixed /></IconBadge><span><strong>Location sharing</strong><small>Share only when you choose</small></span><ChevronRight /></button><button onClick={()=>navigate("settings")}><IconBadge><Settings /></IconBadge><span><strong>Settings</strong><small>Language, text size and comfort</small></span><ChevronRight /></button></div><div className="official-note"><ShieldCheck /><div><strong>SILIRUAL is a daily wellbeing companion</strong><p>It does not provide medical advice or diagnosis.</p></div></div></>; }

function SettingsView({textSize,setTextSize,onLogout}:{textSize:TextSize;setTextSize:(s:TextSize)=>void;onLogout:()=>void}) { const [sound,setSound]=useState(true); return <><PageHeader title="Settings" subtitle="Make SILIRUAL comfortable for you." /><section className="settings-section"><h2>Text size</h2><div className="size-picker">{(["normal","large","extra"] as TextSize[]).map(s=><button key={s} className={textSize===s?"selected":""} onClick={()=>setTextSize(s)}><span className={`sample-${s}`}>Aa</span><small>{{normal:"Normal",large:"Large",extra:"Extra Large"}[s]}</small></button>)}</div></section><section className="settings-section"><ToggleRow icon={<Volume2/>} title="Sound" text="Hear helpful sounds" value={sound} onChange={()=>setSound(!sound)}/><ToggleRow icon={<Mic/>} title="Voice instructions" text="Hear instructions read aloud" value={false} onChange={()=>{}}/><ToggleRow icon={<Accessibility/>} title="High contrast" text="Make colours easier to see" value={false} onChange={()=>{}}/></section><ActionButton variant="danger" onClick={onLogout}>Log out</ActionButton></>; }

function CareView() { return <><PageHeader title="My care circle" subtitle="People you trust and have approved." /><article className="person-card"><div className="person-avatar">P</div><div><h2>Priya Das</h2><p>Daughter · Family</p><span><Check/> Connected</span></div><ChevronRight/></article><ActionButton variant="secondary"><Plus/> Connect a trusted person</ActionButton><div className="reassurance"><ShieldCheck/><div><strong>You decide who can connect</strong><p>You can remove access at any time.</p></div></div></>; }
